...
export class ConcertService {
  ...
  concerts: { id: number; name: string; des: string; amount: string; user_reserve: { userid: number, username: string }[] }[] = [
    { 
      id: 1, 
      name: "Concert Name 1", 
      ...
      amount: "500", 
      user_reserve: [{userid:1,username:"John"}, {userid:2,username:"Lisa"}]
    },
    { 
      id: 2, 
      name: "Concert Name 2", 
  ...
  noti: { time: string, User_name:string, Concertname:string,Action:string}[] =[
  ...
  create(createConcertDto: CreateConcertDto) {
      ...
      const user_reserves = [];
      this.concerts.push({ id: newId, name, des, amount, user_reserve: user_reserves });
...
//2024-03-19T12:28:34.467Z
  ...
  update(id: number, updateConcertDto:UpdateConcertDto){
    ...
    if (concert) {
      const isUserReserved = concert.user_reserve.some(user => user.userid === updateConcertDto.userid);
      if (isUserReserved) {
        return ` ${updateConcertDto.username} is already reserved for this concert`;
      }
  ...
  cancle(id:number ,updateConcertDto:UpdateConcertDto){
    const { userid } = updateConcertDto;
      ...
      if (concert) {
        const isUserReserved = concert.user_reserve.some(user => user.userid === updateConcertDto.userid);
        if (!isUserReserved) {
          return `User is not reserved for this concert`;
        }
        const usernamenoti = concert.user_reserve.find(usernamen => usernamen.userid === userid)
        this.noti.push({
          time: times, 
        ...
        const index = concert.user_reserve.findIndex(item => (item.userid === userid))
  ...
  delete(id: number) {
    ...
    if (index !== -1) {
      ...
      return `Concert #${id} has been removed`;
    } else {
      return `No concert found with ID ${id}`;
    }
  }
